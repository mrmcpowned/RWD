...
//                    document.querySelector('#gamepads').appendChild(el);
    ...
    queueWheel: function (value, gamepadId, id) {

        tester.updateQueue.enqueue([Date.now() + tester.DELAY_TIME_MS, tester.updateWheel, [value, gamepadId, id]]);
        tester.processQueue();
    },
    
    queueShifter: function (value, gamepadId, id, direction) {
        //copy value ... because reference
        var newVal = jQuery.extend({}, value);

        tester.updateQueue.enqueue([Date.now() + tester.DELAY_TIME_MS, tester.updateShifter, [newVal, gamepadId, id, direction]]);
        tester.processQueue();
    },
    
    queueKnob: function (value, gamepadId, id, isClockwise) {
        //copy value ... because reference
        var newVal = jQuery.extend({}, value);

        tester.updateQueue.enqueue([Date.now() + tester.DELAY_TIME_MS, tester.updateKnob, [newVal, gamepadId, id, isClockwise]]);
        tester.processQueue();
    },
    
    queuePedal: function (value, gamepadId, id) {
        //copy value ... because reference
        tester.updateQueue.enqueue([Date.now() + tester.DELAY_TIME_MS, tester.updatePedal, [value, gamepadId, id]]);
        tester.processQueue();
    },
    ...
    updateWheel: function (value, gamepadId, id) {
        var gamepadEl = document.querySelector('#gamepad-' + gamepadId);
        var newValue = value;
        if (typeof value == 'object') {
            var newValue = value.value;
        ...
        var buttonEl = gamepadEl.querySelector('[data-name="' + id + '"]');
        if (buttonEl) { // Extraneous buttons have just a label.
            buttonEl.style.transform = "rotate(" + newValue * 450 + "deg)";
        }

    },    
    
    updateShifter: function (value, gamepadId, id, direction) {
        var gamepadEl = document.querySelector('#gamepad-' + gamepadId);
        var newValue = value;
        if (typeof value == 'object') {
            var newValue = value.value;
        ...
        if (buttonEl) { // Extraneous buttons have just a label.
            if (newValue > tester.ANALOGUE_BUTTON_THRESHOLD) {
                buttonEl.classList.add(direction);
            } else {
                buttonEl.classList.remove(direction);
            }
        }

    },
    
    updateKnob: function (value, gamepadId, id, isClockwise) {
        var gamepadEl = document.querySelector('#gamepad-' + gamepadId);
        var newValue = value;
        if (typeof value == 'object') {
            var newValue = value.value;
        ...
        var buttonEl = gamepadEl.querySelector('[data-name="' + id + '"]');
        if (buttonEl && newValue > 0) { // Extraneous buttons have just a label.
            var rot = parseInt(buttonEl.getAttribute('data-rotation'));
            rot = (isClockwise) ? rot + 15 : rot - 15;
            buttonEl.setAttribute('data-rotation', rot);
            buttonEl.style.transform = "rotate(" + rot + "deg)";
            
        }

    },
    
    updatePedal: function (value, gamepadId, id) {
        var gamepadEl = document.querySelector('#gamepad-' + gamepadId);
        var newValue = value;
        if (typeof value == 'object') {
            var newValue = value.value;
        ...
        if (buttonEl) { // Extraneous buttons have just a label.
            //Hardcoded for now
            newValue = ((newValue * -1) + 1) / 2;
            buttonEl.style.transform = 'rotateX(' + newValue * 75 + 'deg)';
        }

    },    

    /**
     * Update the fight stick using the name provided
     */
    ...
    processQueue: function () {
        ...
        while (tester.updateQueue != undefined && tester.updateQueue.peek() != undefined && tester.updateQueue.peek()[0] <= Date.now()) {
            ...
            var vars = elem[2];
            if (elem[1] == tester.updateStick || elem[1] == tester.updateAxis || elem[1] == tester.updateShifter || elem[1] == tester.updateKnob) {
                elem[1](vars[0], vars[1], vars[2], vars[3])
            } else {
